# ElGamal-криптосистема (TelegramBot)
1.  **Пользователь А** выбирает большое простое число `p`
2.  **Пользователь А** выбирает первообразный элемент `g` по модулю `p`
3.  **Пользователь А** вычисляет секретный ключ: `y = g^a mod p` (где `a` является закрытым)
4.  Открытые параметры `(p, g, y)` отправляются по открытому каналу

### Кодирование сообщения
Сообщения на кириллице преобразуются в 5-битное двоичное представление в соответствии с таблицей соответствия:

| Буква | Двоичный | Буква | Двоичный | Буква | Двоичный | Буква | Двоичный |
|--------|--------|--------|--------|--------|--------|--------|--------|
...
### Процесс шифрования (Пользователь Б)
1.  Выбрать случайное целое число `k` (одноразовый/эфимерный ключ)
2.  Вычислить `y₁ = g^k mod p`
3.  Вычислить общий секрет: `key = y^k mod p` (где `y` — открытый ключ Пользователя А)
4.  Преобразовать `key` в двоичное представление
5.  Вычислить `y₂ = m ⊕ key` (операция XOR)
6.  Отправить шифротекст `(y₁, y₂)` Пользователю А

### Процесс расшифрования (Пользователь А)
Получив шифротекст `(y₁, y₂)`:
1.  Вычислить общий секрет: `s = y₁^a mod p` (используя закрытый ключ `a`)
2.  Преобразовать `s` в двоичное представление
3.  Восстановить сообщение: `m = y₂ ⊕ s`
